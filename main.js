...
document.querySelector("#homeform").addEventListener("submit", (event) => {
  ...
  if (searchInput.length > 0) {
    ...
    showErrorMessage(
      "Please enter the name of a town or county to search for."
    );
  }
});
...
document.querySelector("#pullutionform").addEventListener("submit", (event) => {
  ...
  if (searchInput.length > 0) {
    ...
    getCurrentPollutionByCity(searchInput, 1).then(
      buildCurrentPollutionCallback
    );

    // Prognos för föroreningar
    getPollutionForecastByCity(searchInput).then(
      buildPollutionForecastCallback
    );
  } else {
    showErrorMessage(
...
document.querySelector("#contactform").addEventListener("submit", (event) => {
  ...
  contactFormMessage.innerText =
    "Thank you for your message, we will answer shortly.";
  contactForm.reset();
  event.preventDefault();
